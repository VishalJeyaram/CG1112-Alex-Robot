...
#define ALEX_P_KEY  "alex.key"
#define ALEX_CERT   "alex.crt"
#define LAPTOP      "laptop.epp.com"
...
void sendNetworkData(const char *data, int len)
{
	...
	if(networkActive)
	{
		...
		while(!exitW){
			...
			int len = sslWrite(tls_conn, buffer, sizeof(buffer));
...
int main()
{
	...
	createServer(ALEX_P_KEY,ALEX_CERT, SERVER_PORT, &worker, CA_CERT, LAPTOP, 1);	
    /* TODO END */
